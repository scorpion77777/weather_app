$(document).ready(function () {
  ...
  var apiKey = "&appid=e88cb3ff66f8d540caf49e9346984117";
  ...
  function loadHistory() {
    ...
    for (let i = 0; i < history.length; i++) {
      ul.append(
        `<li><button style="margin: 1rem; color:white; border:none;" class="btn  btn-outline-dark" id="${history[i]}" onClick="reply_click(this.id)">${history[i]}</button></li>`
      );
    }
  }
  loadHistory();
        ...
        response.json().then(function (data) {
          ...
          function displayForecast(data, number, dayNumber) {
            forecastIcon =
              ...
              data.list[dayNumber].weather[0].icon +
              ".png";
            windForecastEl[number].text(
              "Wind: " + data.list[dayNumber].wind.speed + " MPH"
            );
            tempForecastEl[number].text(
              "Temp: " + data.list[dayNumber].main.temp + " °F"
            );
            dateForecastEl[number].text(
              moment(data.list[dayNumber].dt_txt).format("M/DD/YY")
            );
            humidityForecastEl[number].text(
              "Humidity: " + data.list[dayNumber].main.humidity + " %"
            );
      ...
      if (response.ok) {
        ...
        response.json().then(function (data) {
          ...
          var uviCall =
            ...
            apiKey;
          fetch(uviCall).then(function (res) {
            if (res.ok) {
              console.log(res);
...
function reply_click(clicked_id) {
  ...
  document.getElementById("search-input").value = buttonID;
